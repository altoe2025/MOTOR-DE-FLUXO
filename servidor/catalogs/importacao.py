"""Carregamento único e determinístico do catálogo de importação."""
...
def _read_catalog(path: Path | None) -> dict[str, Any]:
    if path is None:
        content = files("servidor.catalogs").joinpath(
            "importacao.v1.json"
...
def load_import_catalog(path: Path | None = None) -> CatalogoImportacao:
    ...
    digest = hashlib.sha256(canonical_catalog_bytes(document)).hexdigest()
    return CatalogoImportacao.model_validate(
        {**document, "catalog_version": digest}
    )
