"""DTOs estritos para preparar carteiras antes da execução de prévia."""
...
def required_source_paths(input_value: EffectiveInput) -> set[str]:
    """Retorna os caminhos estáveis que exigem origem na entrada efetiva."""
...
class EffectiveInput(StrictModel):
    ...
    def validate_semantics(self) -> EffectiveInput:
        ...
        if any(
            rule.finalidade != rule.finalidade.strip()
            for rule in self.costs.iof_por_finalidade
        ):
