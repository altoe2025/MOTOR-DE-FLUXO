...
def _is_uncollected_efx_pointer(pointer: str) -> bool:
    return _UNCOLLECTED_EFX_POINTER_RE.fullmatch(pointer) is not None
    ...
    def validate_cross_fields(self) -> PreviaRequest:
        ...
        for pointer, origin in self.proveniencia.items():
            if origin.tipo != "NAO_COLETADO":
                continue
            if not _is_uncollected_efx_pointer(pointer):
                raise ValueError(
