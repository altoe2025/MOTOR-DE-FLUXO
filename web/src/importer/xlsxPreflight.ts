...
const EXPECTED_HEADERS = ['operacao_id', 'cliente_nome', 'classificacao_perfil', 'direcao', 'data_conhecida', 'data_limite', 'valor_brl', 'finalidade_codigo'];
...
function inspectWorksheet(xmlText: string, strings: string[]): void { const headers: Array<string | undefined> = []; let row = 0; let ref = ''; let type = ''; let readingValue = false; let inline = false; let value = ''; parseXml(xmlText, { open(name, attributes) { if (name === 'f') fail('FORMULA_NOT_ALLOWED', 'fórmulas não são permitidas'); if (name === 'mergeCell') fail('MERGED_CELLS_NOT_ALLOWED', 'células mescladas não são permitidas'); if (name === 'row') row = Number(attributes.r ?? 0); else if (name === 'c') { if (row > MAX_WORKSHEET_ROW) fail('ROW_LIMIT_EXCEEDED', 'o XLSX excede 1.000 operações'); ref = attributes.r ?? ''; type = attributes.t ?? ''; value = ''; } else if (name === 'v') readingValue = true; else if (name === 't' && type === 'inlineStr') inline = true; }, close(name) { if (name === 'v') readingValue = false; else if (name === 't') inline = false; else if (name === 'c' && row === 1) { const index = column(ref); if (index >= 0) headers[index] = type === 's' ? strings[Number(value)] : value; } }, text(text) { if (readingValue || inline) value += text; } }); if (headers.length !== EXPECTED_HEADERS.length || headers.some((header, index) => header !== EXPECTED_HEADERS[index])) fail('HEADER_INVALID', 'headers fora da ordem canônica'); }
