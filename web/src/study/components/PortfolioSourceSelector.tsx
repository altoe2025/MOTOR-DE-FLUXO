...
import type { ScenarioDocument, StudyDocument } from '../model';
...
export type PortfolioSourceDraft =
  ...
  | Readonly<{ kind: 'AUTHORED'; authoredPortfolioId: string; preparation: PreparationRequest }>
...
type Participant = { id: string; name: string; override: boolean; parameters: Parameters };
type Group = { id: string; name: string; parameters: Parameters; participants: Participant[] };
...
function expectedBuildSha(scenario: ScenarioDocument): string {
  if (scenario.sourceSnapshot.source.kind === 'SYNTHETIC') {
    return scenario.sourceSnapshot.source.recipe.motorBuildSha;
  }
  return import.meta.env.VITE_MOTOR_BUILD_SHA ?? '0'.repeat(40);
}
...
function request(study: StudyDocument, scenario: ScenarioDocument, groups: readonly Group[], sourceKind: 'PADRAO_SINTETICO' | 'ESTIMATIVA_USUARIO'): PreparationRequest {
  const participants = groups.flatMap((item) => item.participants.map((member) => {
    ...
    const deadline = decimal(values.deadline, 'Prazo');
  ...
  return {
    ...
    input: {
      ...
      sources: { '/orders': { kind: sourceKind, source: sourceKind === 'PADRAO_SINTETICO' ? 'catálogo oficial de exemplos' : 'autoria manual no editor', recorded_at: new Date().toISOString() } },
    },
  };
}
...
function AuthoredForm({ study, scenario, onApply, onDirty }: { study: StudyDocument; scenario: ScenarioDocument; onApply(source: PortfolioSourceDraft): void; onDirty(dirty: boolean): void }) {
  ...
  const submit = () => {
    try {
      ...
      onApply({ kind: 'AUTHORED', authoredPortfolioId: uuid(), preparation });
...
function SyntheticForm({ study, scenario, onApply }: { study: StudyDocument; scenario: ScenarioDocument; onApply(source: PortfolioSourceDraft): void }) {
