...
export function StudyList({ studies, selectedId, onCreate, onOpen, onRename, onDuplicate, onRestore, onDelete }: StudyListProps) {
  const visibleStudies = studies.filter((study) => study.deletedAt === null);
  return <><div className="list-toolbar"><Button onClick={onCreate}>Novo estudo</Button></div>{visibleStudies.length === 0 ? <p className="empty-list">Nenhum estudo salvo nesta conta.</p> : <ul className="study-list" aria-label="Estudos">{visibleStudies.map((study) => {
