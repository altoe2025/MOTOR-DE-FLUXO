...
ponto (mix, N, W) sob várias seeds e resume o eixo de seeds em mediana e faixa
(`_mediana`, `_percentil` com `p25`/`p50`/`p75`, em `motor/varredura.py`). O
...
são `p25`/`p50`/`p75`, não `p10`. Se a intenção é formalizar esse racional (p10 como o
percentil relevante para decisão sob risco), isso é uma mudança de código
(`motor/varredura.py:_percentil`, chamado com `Decimal("0.10")`) a ser proposta
como tarefa própria, não uma decisão já tomada.
...
- Antes de reportar qualquer conclusão do tipo "N reduz risco", confirmar contra o
  CSV de saída real (`taxa_netabilidade_p25`/`p50`/`p75` — não existe coluna `p10`
  hoje) em vez de assumir um percentil que o código não calcula.
